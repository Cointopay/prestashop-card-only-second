...
class Cointopay_Direct_Cc_Second extends PaymentModule
{
    public $merchant_id;
    ...
    {
        $this->name = 'cointopay_direct_cc_second';
        ...
        $config = Configuration::getMultiple(
            array(
                'COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID',
                'COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE',
                'COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY',
                'COINTOPAY_DIRECT_CC_SECOND_DISPLAY_NAME'
            )
        );

        if (!empty($config['COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID'])) {
            $this->merhcant_id = $config['COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID'];
        }
        if (!empty($config['COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE'])) {
            $this->security_code = $config['COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE'];
        }
        if (!empty($config['COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY'])) {
            $this->crypto_currency = $config['COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY'];
        }

        parent::__construct();

        $this->displayName = 'Cointopay Direct CC Second';
    ...
    public function uninstall()
    {
        ...
        return (
            Configuration::deleteByName('COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID') &&
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            if (!Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID')) {
                $this->postErrors[] = $this->l('Merchant id is required.');
            }

            if (!Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE')) {
                $this->postErrors[] = $this->l('Security Code is required.');
            }

            if (!Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY')) {
                $this->postErrors[] = $this->l('Checkout Currency is required.');
            }

            if (empty($this->postErrors)) {
                $ctpConfig = array(
                    'merchant_id' => Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID'),
                    'security_code' => Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE'),
                    'selected_currency' => Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY'),
                    'user_agent' => 'Cointopay - Prestashop v' . _PS_VERSION_
                        . ' Extension v' . COINTOPAY_DIRECT_CC_SECOND_PRESTASHOP_EXTENSION_VERSION,
                );

                \Cointopay_Direct_Cc_Second\Cointopay_Direct_Cc_Second::config($ctpConfig);

                $merchant = \Cointopay_Direct_Cc_Second\Cointopay_Direct_Cc_Second::verifyMerchant();
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            Configuration::updateValue('COINTOPAY_DIRECT_CC_SECOND_DISPLAY_NAME', Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_DISPLAY_NAME'));
            Configuration::updateValue('COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID', Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID'));
            Configuration::updateValue('COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE', Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE'));
            Configuration::updateValue('COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY', Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY'));
        }
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'name' => 'COINTOPAY_DIRECT_CC_SECOND_DISPLAY_NAME',
                        'required' => true,
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->l('Merchant ID'),
                        'name' => 'COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID',
                    ...
                    array(
                        ...
                        'name' => 'COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE',
                    ...
                    array(
                        ...
                        'name' => 'COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY',
                        'id' => 'crypto_currency',
                        'default_value' => (int)Tools::getValue('COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY'),
                        'required' => true,
                        'options' => array(
                            'query' => $options,
                            'id' => 'id_option',
    ...
    {
        $system_name = Configuration::get('COINTOPAY_DIRECT_CC_SECOND_DISPLAY_NAME');
        $dislay_name = (isset($system_name) && !empty($system_name)) ? $system_name : 'Cointopay Direct CC Second';

        return array(
            'COINTOPAY_DIRECT_CC_SECOND_DISPLAY_NAME' => $dislay_name,
            'COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID' => Configuration::get('COINTOPAY_DIRECT_CC_SECOND_MERCHANT_ID'),
            'COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE' => Configuration::get('COINTOPAY_DIRECT_CC_SECOND_SECURITY_CODE'),
            'COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY' => Configuration::get('COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY'),
        );
    }
    ...
    private function displayCointopayInformation($renderForm)
    {
        ...
        $this->context->smarty->assign("selected_currency", Configuration::get('COINTOPAY_DIRECT_CC_SECOND_CRYPTO_CURRENCY'));
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (isset($_REQUEST['CustomerReferenceNr'])) {
            ...
            return $this->context->smarty->fetch('module:cointopay_direct_cc_second/views/templates/hook/ctp_success_callback.tpl');
        }
    }
    ...
    public function hookPaymentOptions($params)
    {
		...
		->setAction($this->context->link->getModuleLink($this->name, 'validation', array(), true))
            ->setAdditionalInformation(
                $this->context->smarty->fetch('module:cointopay_direct_cc_second/views/templates/hook/cointopay_intro.tpl')
            )
    ...
    public function hookDisplayAdminOrderLeftMain($order)
    {
        ...
        return '<div class="panel" id="cointopaydirectccsecondform">
                    <div class="panel-heading">Payment Page('.$this->displayName.')</div>
                    <div>Payment Page For this Order:<br/><span id="ctpdirectccsecondpurl">'.$paymentUrl.'</span></div>
					<form method="post" action="" style="display:inline-block;">
                    <input type="submit" class="btn btn-outline-secondary" name="send'.$this->name.'Payment" value="Send To Customer" />
					</form><button id="ctpdirectccsecondcopytext" class="btn btn-outline-secondary" onclick="ctpdirectccsecondCopyFunction()" style="margin-left:10px;">Copy URL to clipboard</button>
                </div> <script>
                    function ctpdirectccsecondCopyFunction() {
					  var copyText = document.getElementById("ctpdirectccsecondpurl");
